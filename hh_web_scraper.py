# -*- coding: utf-8 -*-
###########################################################################
# На первом шаге функция возвращает количество страниц в запросе и вызывает вторую функцию
# 
# Вторая функция вытаскивает с каждой страниц список ссылок на каждую вакансию и  возвращает список всех ссылок
# 
# На третьем шаге функция проходит по каждой ссылке, считывает описание вакансии и обрабатывает –
# По возможности идет только в раздел «Требования» и считает количество слов оттуда
# Записывает в словарь, где ключ будет само слово (в нижнем регистре), а значением будет количество слов в тексте
# Если встретили слово и оно есть в словаре, то увеличиваем его количество
# Если встретили слово и его нет в словаре, то добавляем слово в словарь и значение количества = 1
# Может несколько словарей для раздела «Требования», «Описание вакансии», «Условия», «Зарплата», чтобы в целом представлять, что чаще всего предлагают работодатели, что требуют, какие условия, какая зарплата предлагается.
#
# Результат: количество наиболее часто встречающихся слов в вакансиях по заданному клчевому слову (можно также задавать регион и т.д.)
###########################################################################
...
# для использования просто запустить функцию, и создастся файл
###########################################################################
...
def get_and_save_area_codes():
    html = get_html('https://hh.ru/search/vacancy')
    time.sleep(.3)
    soup = BeautifulSoup(html, 'lxml')
    # areas_parsed = []
    ...
    with open('area_codes.txt', 'w', encoding='utf-8') as f:
        for pair in pairs:
